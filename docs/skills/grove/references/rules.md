...
**Lazy distillation.** Distillation is NOT auto-created on `status(g) = verified`. When **`W → done`** pushes a linked goal to **`verified`**, the CLI prints a one-line **stderr** reminder to run `grove distill G-NN` (worksheet) or `grove distill G-NN --null` (nothing worth keeping) when ready. Add a **`notes`** line containing `--distill-deferred` on the goal to suppress the reminder ([checklist.md](checklist.md)).
